...
> **Local install note:** this copy vendors macOS static builds of `yt-dlp`,
> `ffmpeg`, and `ffprobe` under `bin/` (not part of upstream) so setup works
> without Homebrew — see `bin/PROVENANCE.md` for exact versions, checksums,
> and how they were verified. `scripts/binpath.py` prefers `bin/` on macOS and
> falls back to `PATH` everywhere else, so the behavior described below is
> unchanged — only *where* the binaries come from differs from upstream.

**What this skill does:**
- Runs `yt-dlp` locally (vendored macOS binary, or `PATH`) to download the video and pull native captions when the source supports them (public data; the request goes directly to whatever host the URL points at)
- Runs `ffmpeg` / `ffprobe` locally (vendored macOS binaries, or `PATH`) to extract frames as JPEGs and, when Whisper is needed, a mono 16 kHz audio clip
