...
from binpath import resolve as _resolve_bin  # noqa: E402
...
def _which(name: str) -> str | None:
    """Resolve a system tool (e.g. brew) via PATH only — not vendor-aware."""
    return shutil.which(name)


def _check_binaries() -> list[str]:
    """Missing binaries, vendor-aware: checks bin/ (macOS) before PATH."""
    return [b for b in REQUIRED_BINARIES if not _resolve_bin(b)]
