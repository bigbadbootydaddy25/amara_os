...
export function AmaraContainer() {
  ...
  useEffect(() => {
    if (isDemoMode || !isActivated || !isVoiceSupported) {
      return;
    }
  ...
  return (
    <main className="relative h-screen w-screen overflow-hidden bg-black">
      ...
      {error ? (
        <div className="pointer-events-none fixed left-1/2 top-8 z-30 max-w-xl -translate-x-1/2 rounded-full border border-white/10 bg-black/45 px-4 py-2 text-center text-[10px] font-mono uppercase tracking-[0.24em] text-white/72 backdrop-blur-md">
          {error}
        </div>
      ) : null}
