...
export class OrbRenderer {
  ...
  update(deltaTime: number, amaraState: AmaraState, audioLevel: number): void {
    ...
    const stateOrbitFactor =
      amaraState === 'idle'       ? 1
      : amaraState === 'listening'  ? 0.86
      : amaraState === 'thinking'   ? 1.16
      : amaraState === 'ingesting'  ? 1.4
      : amaraState === 'alert'      ? 1.2
      : 0.94;
    const stateSpeedFactor =
      amaraState === 'idle'       ? 0.45
      : amaraState === 'listening'  ? 0.75
      : amaraState === 'thinking'   ? 1.35
      : amaraState === 'ingesting'  ? 2.8
      : amaraState === 'alert'      ? 2.0
      : 0.95;
    ...
    this.orbs.forEach((orb, index) => {
      ...
      const baseRadiusFactor =
        amaraState === 'idle'       ? 0.88
        : amaraState === 'listening'  ? 0.98 + audioBoost * 0.35
        : amaraState === 'thinking'   ? 1.08 + contractionPulse * 0.22
        : amaraState === 'ingesting'  ? 1.3 + Math.sin(this.time * 3.5 + orb.phase) * 0.2
        : amaraState === 'alert'      ? 1.5 + Math.sin(this.time * 8) * 0.3
        : 1.06 + audioBoost * 0.5;
      ...
      const baseOpacity =
        amaraState === 'idle'       ? 0.13
        : amaraState === 'listening'  ? 0.2
        : amaraState === 'thinking'   ? 0.26
        : amaraState === 'ingesting'  ? 0.38
        : amaraState === 'alert'      ? 0.55
        : 0.24;
      const sparkle = (Math.sin(this.time * 2.2 + orb.phase * 17) + 1) * 0.5;
      const opacityBoost =
        amaraState === 'speaking'   ? audioBoost * 0.22
        : amaraState === 'thinking'   ? contractionPulse * 0.18 + sparkle * 0.08
        : amaraState === 'listening'  ? audioBoost * 0.12
        : amaraState === 'ingesting'  ? sparkle * 0.25 + Math.sin(this.time * 4 + orb.phase * 3) * 0.1
        : sparkle * 0.03;
      orb.opacity = clamp(baseOpacity + opacityBoost + (index % 4 === 0 ? 0.08 : 0), 0.08, 0.72);
    });
  }
  ...
  render(): void {
    ...
    for (const orb of this.orbs) {
      // State-driven hue override
      const hue =
        amaraState === 'listening'  ? 195                                    // cyan #00D4FF
        : amaraState === 'thinking'   ? 42                                   // amber #FFB800
        : amaraState === 'ingesting'  ? (orb.hue + this.time * 60) % 360    // full multi-color rotation
        : amaraState === 'alert'      ? 0                                    // red #FF3B3B
        : orb.hue;

      const glow = ctx.createRadialGradient(orb.x, orb.y, 0, orb.x, orb.y, orb.radius * 2.6);
      glow.addColorStop(0, `hsla(${hue}, 88%, 72%, ${orb.opacity})`);
      glow.addColorStop(0.28, `hsla(${hue}, 88%, 65%, ${orb.opacity * 0.5})`);
      glow.addColorStop(1, `hsla(${hue}, 88%, 50%, 0)`);
      ctx.fillStyle = glow;
      ctx.beginPath();
