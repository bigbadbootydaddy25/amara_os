...
class ClaudeObsidian:
    """LLM-powered Obsidian vault integration."""
    ...
    def create_linked_note(self, title: str, context: str) -> Path:
        ...
        content = llm.think(
            ...
            "Format: frontmatter (tags, date), then headers and content. Be concise and factual.",
            depth=2,
        )
    ...
    def append_to_note(self, title: str, new_content: str) -> Path:
        ...
        summary = llm.reason_fast(
            f"Summarise the following for an Obsidian note section:\n\n{new_content}"
        )
        updated = existing + f"\n\n## Added\n{summary}"
    ...
    def suggest_connections(self, note_title: str) -> list[str]:
        ...
        candidates = [t for t in self._list_note_titles() if t != note_title]
        if not candidates:
            return []
        prompt = (
        ...
        lines = llm.think(prompt).strip().splitlines()
    ...
    def _cluster_topics(self, notes: list[dict[str, Any]]) -> list[str]:
        ...
        return [
            l.strip("- ").strip()
            for l in llm.think(prompt).strip().splitlines()
            if l.strip()
        ]
    ...
    def _load_all_notes(self) -> list[dict[str, Any]]:
        if not self._vault.exists():
            return []
        return [
            {"title": p.stem, "path": str(p), "content": p.read_text(encoding="utf-8", errors="ignore")}
