...
export class AvatarRenderer {
  ...
  render(): void {
    ...
    const neckWidth = headWidth * 0.55;
  ...
  private drawHead(headWidth: number, headHeight: number): void {
    ...
    headPath.quadraticCurveTo(0, headHeight * 0.62, -headWidth * 0.28, headHeight * 0.47);
    headPath.bezierCurveTo(
      -headWidth * 0.48,
      headHeight * 0.2,
      -headWidth * 0.52,
    ...
    const cheekHighlight = ctx.createLinearGradient(-headWidth * 0.24, 0, headWidth * 0.24, 0);
    ...
    ctx.beginPath();
    ctx.ellipse(0, headHeight * 0.04, headWidth * 0.26, headHeight * 0.17, 0, 0, Math.PI * 2);
    ctx.fill();

    const rimGradient = ctx.createRadialGradient(0, -headHeight * 0.06, headWidth * 0.05, 0, 0, headWidth * 0.62);
  ...
  private drawPanelLines(headWidth: number, headHeight: number): void {
    ...
    ctx.beginPath();
    ctx.moveTo(-headWidth * 0.28, -headHeight * 0.16);
    ctx.quadraticCurveTo(-headWidth * 0.46, headHeight * 0.04, -headWidth * 0.26, headHeight * 0.34);
    ctx.stroke();
    ...
    ctx.beginPath();
    ctx.moveTo(-headWidth * 0.24, headHeight * 0.22);
    ctx.quadraticCurveTo(0, headHeight * 0.28, headWidth * 0.24, headHeight * 0.22);
    ctx.stroke();

    ctx.strokeStyle = 'rgba(72, 78, 88, 0.24)';
  ...
  private drawEyes(headWidth: number, headHeight: number): void {
    ...
    [-1, 1].forEach((direction) => {
      ...
      socketGradient.addColorStop(0, 'rgba(25, 32, 54, 0.95)');
      socketGradient.addColorStop(0.6, 'rgba(9, 13, 23, 0.92)');
  ...
  private drawMouth(headWidth: number, headHeight: number): void {
    ...
    const mouthWidth = headWidth * 0.28;
    ...
    interiorGradient.addColorStop(0, '#22304f');
    interiorGradient.addColorStop(0.2, '#1a1a2e');
    interiorGradient.addColorStop(1, '#080b16');
    ...
    if (this.animState.mouthOpenness > 0.1) {
      ctx.strokeStyle = `rgba(93, 190, 255, ${0.15 + this.animState.mouthOpenness * 0.2})`;
      ctx.lineWidth = Math.max(1, headWidth * 0.004);
      for (let i = -1; i <= 1; i += 1) {
      ...
      glowGradient.addColorStop(0, `rgba(0, 200, 255, ${0.1 + this.animState.mouthOpenness * 0.16})`);
  ...
  private drawNeck(neckWidth: number, neckHeight: number, headHeight: number): void {
    ...
    ctx.beginPath();
    ctx.roundRect(-neckWidth * 0.5, neckTop, neckWidth, neckHeight * 1.55, neckWidth * 0.12);
    ctx.fill();

    ctx.strokeStyle = 'rgba(74, 82, 96, 0.42)';
    ctx.lineWidth = Math.max(1.2, neckWidth * 0.014);
    ctx.beginPath();
    ctx.moveTo(-neckWidth * 0.5, neckTop + neckHeight * 0.18);
    ctx.lineTo(neckWidth * 0.5, neckTop + neckHeight * 0.18);
    ctx.stroke();

    ctx.strokeStyle = 'rgba(45, 52, 68, 0.55)';
    ctx.lineWidth = Math.max(1, neckWidth * 0.012);
